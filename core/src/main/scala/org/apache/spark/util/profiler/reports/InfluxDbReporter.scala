...
private[spark] class InfluxDbReporter extends Reporter {
  ...
  lazy private val writeApi = client.map(_.getWriteApi)
  ...
  def report(events: Seq[(Map[String, String], Event)]): Unit = Future {
    writeApi match {
      case Some(writeApi) =>
        val points = events.map(e => constructPoint(e._1, e._2)).toList
        try {
          logger.trace(s"Sending line to influx db: ${points.map(_.toLineProtocol).mkString(", ")}")
          writeApi.writePoints(bucket, org, points.asJava)
        }
        catch {
  ...
  private def constructPoint(context: Map[String, String], event: Event): Point = {
    val functionName = context.getOrElse("function", "Unknown Function").toString.replace(" ", "_")
    val memoryUsage = Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()
    Point
      .measurement(s"${functionName}_$event")
      .addTags(context.filterNot{case (_, v) => v == null}.asJava)
      .time(
        context.getOrElse("systemTimeMs", System.currentTimeMillis().toString).toLong,
        WritePrecision.MS
      )
